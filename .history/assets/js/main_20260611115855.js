...
function initProjectScrollDots() {
  ...
  dots.forEach((dot, index) => {
    dot.addEventListener('click', () => {
      const cardWidth = cards[0]?.offsetWidth || 1;
      const gapWidth = 20;
      const targetTranslate = -(index * (cardWidth + gapWidth));
      const maxTranslate = Math.max(0, track.scrollWidth - container.offsetWidth);
      const clampedTranslate = Math.max(-maxTranslate, targetTranslate);
      const scrollPercent = Math.min(1, Math.abs(clampedTranslate) / (maxTranslate || 1));

      const rect = portfolio.getBoundingClientRect();
      const height = rect.height;
      const scrollable = height - window.innerHeight;
      const targetScrollY = window.scrollY + scrollPercent * Math.max(0, scrollable);

      if (typeof window._scrollTo === 'function') {
        window._scrollTo(Math.max(0, targetScrollY));
      } else {
        window.scrollTo({
